    ...
    for input in siteList:
        ...
        for n in range(0,sampleRecords.size().getInfo()) : 
            ...
            if ( defaultDate==False ):
                startDate = datetime.fromtimestamp(ee.Date.parse('dd/MM/YY',site.get('system:time_start')).getInfo()['value']/1000) 
                if ("system:time_end" in site.propertyNames().getInfo()):
                    endDate = datetime.fromtimestamp(ee.Date.parse('dd/MM/YY',site.get('system:time_end')).getInfo()['value']/1000) 
...
def imageSites(siteList,imageCollectionName,algorithm,variableName='LAI',maxCloudcover=0,outputScaleSize=0,inputScaleSize=30,bufferSpatialSize=0,bufferTemporalSize=[0,0],subsamplingFraction=1):
    ...
    for input in siteList:
        ...
        for n in range(0,sampleRecords.size().getInfo()) : 
            ...
            startDate = datetime.fromtimestamp(ee.Date.parse('dd/MM/YY',site.get('system:time_start')).advance(bufferTemporalSize[0],'day').getInfo()['value']/1000)
            endDate = datetime.fromtimestamp(ee.Date.parse('dd/MM/YY',site.get('system:time_end')).advance(bufferTemporalSize[1],'day').getInfo()['value']/1000)
            endDatePlusOne = datetime.fromtimestamp(ee.Date.parse('dd/MM/YY',site.get('system:time_end')).advance(bufferTemporalSize[1]+1,'day').getInfo()['value']/1000)
